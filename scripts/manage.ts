...
export function discoverDiskModules(): Array<{ name: string; type: "shared" | "feature"; dir: string }> {
  ...
  if (fs.existsSync(sharedDir)) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && !entry.name.startsWith(".")) {
        results.push({ name: entry.name, type: "shared", dir: path.join(sharedDir, entry.name) });
      }
    }
  }
  ...
  if (fs.existsSync(featuresDir)) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && !entry.name.startsWith(".")) {
        results.push({ name: entry.name, type: "feature", dir: path.join(featuresDir, entry.name) });
      }
    }
  }
...
export function checkGitStatusForDir(dirPath: string): { hasChanges: boolean; filesCount: number } {
  try {
...
export function getRegistryDiff(): RegistryModuleStatus[] {
  ...
  const diskMap = new Map<string, { type: "shared" | "feature"; dir: string }>();
  ...
  for (const key of allKeys) {
    ...
    const registeredEntry = (type === "shared" ? index.shared : index.features).find(
      (e) => e.name === name
    );
  ...
  return statuses.sort((a, b) => a.type.localeCompare(b.type) || a.name.localeCompare(b.name));
}
...
export function diffRegistry(): void {
  ...
  const uncommitted = diffs.filter((d) => d.existsOnDisk && d.hasUncommittedGitChanges);
  const committedDrift = diffs.filter(
    (d) => d.existsOnDisk && d.isRegistered && !d.hasUncommittedGitChanges && d.hasIntegrityDrift
  );
  ...
  if (unregistered.length > 0) {
    p.log.message(pc.yellow(`\n📦 Unregistered Modules (${unregistered.length}):`));
    for (const mod of unregistered) {
      p.log.message(`  ${pc.red("●")} ${pc.bold(`${mod.type}/${mod.name}`)} ${pc.dim("(not registered in index.json)")}`);
    }
  }
  ...
  if (missing.length > 0) {
    ...
    for (const mod of missing) {
      p.log.message(`  ${pc.red("✕")} ${pc.bold(`${mod.type}/${mod.name}`)} ${pc.dim("(in index.json, but folder missing)")}`);
    }
  }

  if (upToDate.length > 0) {
    p.log.message(pc.green(`\n✓ Up to Date & Registered (${upToDate.length}):`));
    for (const mod of upToDate) {
      p.log.message(`  ${pc.green("✔")} ${mod.type}/${mod.name} ${pc.dim(`(${mod.registeredIntegrity?.slice(0, 18)}...)`)}`);
    }
  }
...
export async function registerModule(typeArg?: string, nameArg?: string): Promise<void> {
  const index = loadMasterIndex();
  ...
  const registeredNames = new Set((type === "shared" ? index.shared : index.features).map((m) => m.name));
  const unregistered = diskModules.filter((m) => !registeredNames.has(m.name));

  let name = nameArg?.trim();
  if (!name) {
    if (unregistered.length > 0) {
      const opts = unregistered.map((m) => ({ value: m.name, label: m.name, hint: "unregistered on disk" }));
      opts.push({ value: "__custom__", label: "✏️ Enter custom name...", hint: "" });
  ...
  if (!fs.existsSync(modDir)) {
    p.log.error(pc.red(`Module directory does not exist: ${path.relative(rootDir, modDir)}`));
    process.exit(1);
  }
...
export async function commitModule(typeArg?: string, nameArg?: string, messageArg?: string): Promise<void> {
  let type = typeArg?.trim() as "shared" | "feature" | undefined;
  if (!type) {
    type = handleCancel(
      await p.select({
  ...
  if (!name) {
    ...
    const opts = diskModules.map((m) => {
      ...
      return {
        value: m.name,
        label: m.name,
        hint: git.hasChanges ? `${git.filesCount} uncommitted files` : "clean working tree",
      };
    });
  ...
  if (!gitStatus.hasChanges) {
    p.log.warn(pc.yellow(`No uncommitted git changes found in '${type}/${name}'.`));
    return;
  }
  ...
  if (!message) {
    message = handleCancel(
      await p.text({
        ...
        validate: (v) => (v && v.trim() ? undefined : "Commit message cannot be empty"),
      })
    ).trim();
  }
  ...
  try {
    execSync(`git add "${relModDir}"`, { cwd: rootDir, stdio: ["ignore", "pipe", "pipe"] });
    execSync(`git commit -m "${message.replace(/"/g, '\\"')}" -- "${relModDir}"`, {
      cwd: rootDir,
...
export async function registerModuleChanges(typeArg?: string, nameArg?: string, versionArg?: string): Promise<void> {
  const index = loadMasterIndex();
  ...
  if (!name) {
    const diskModules = discoverDiskModules().filter((m) => m.type === type);
    const diffs = getRegistryDiff().filter((d) => d.type === type && d.isRegistered);
    ...
    const opts = diskModules.map((m) => {
      ...
      return {
        value: m.name,
        label: m.name,
        hint: d?.hasIntegrityDrift ? "drift detected (needs register)" : "up to date",
      };
    });
  ...
  p.log.success(
    pc.green(`✓ Registered changes for '${type}/${name}' (new integrity: ${integrity.slice(0, 18)}...)`)
  );
}

export async function unregisterModule(typeArg?: string, nameArg?: string): Promise<void> {
  const index = loadMasterIndex();
